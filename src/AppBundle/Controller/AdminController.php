...
/**
* @Route("/addOmdbSerie/{t}", name="admin_add_omdb_serie")
* @Template()
*/
public function addOmdbSerie($t, Request $request)
{

    $omdb = new OMDbAPI();
    $showToAdd = $omdb->fetch('t', $t);

    $title = $showToAdd->data->Title;
    $synopsis = $showToAdd->data->Plot;
    $image = $showToAdd->data->Poster;
    ...
    if ($showToAdd->data->Poster) {
        $ext = explode(".", $showToAdd->data->Poster);
        $ext = $ext[count($ext)-1];
        ...
        copy($showToAdd->data->Poster, $webRoot.'/uploads/'.$filename);
        $show->setImage($filename);
    }
